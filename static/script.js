document.addEventListener('DOMContentLoaded', () => {
    ...
    saveButton.addEventListener('click', () => {
        if (conversationActive && messages.length > 0) {
            const firstMessage = messages[0].content.substring(0, 20) + (messages[0].content.length > 20 ? '...' : '');
    ...
    fetch('/load_conversations')
        ...
        .then(conversations => {
            conversations.forEach(convo => {
                ...
                const model = convo[1];
                const firstMessage = JSON.parse(convo[2])[0].content.substring(0, 20) + '...';
                ...
                li.addEventListener('click', () => loadConversation(convo[0], model, JSON.parse(convo[2])));
            });
        });
